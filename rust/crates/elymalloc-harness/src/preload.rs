...
pub fn run() -> Result<()> {
    ...
    if !env_is_one("SKIP_BUILD") {
        ...
        let st = cargo_in_root()
            .args(["build", "--release", "-p", "elymalloc-c"])
            .status()?;
        if !st.success() {
            bail!("cargo build elymalloc-c failed");
        }
    }
    let so = so
        .canonicalize()
    ...
    if !env_is_one("RUSTC_ONLY") {
        ...
        println!("==> rustc: rebuild and test elymalloc-core under LD_PRELOAD");
        let err_path = cache.join("cargo-test.err");
        let mut cmd = cargo_in_root();
        cmd.args(["test", "-p", "elymalloc-core", "--offline"])
            .env("LD_PRELOAD", &so)
            .stderr(std::fs::File::create(&err_path)?);
        let ok = cmd.status()?.success();
        if ok {
            println!("  ok   cargo-test-elymalloc-core");
            st.pass += 1;
            st.record("PASS", "cargo-test-elymalloc-core");
        } else {
            println!("  FAIL cargo-test-elymalloc-core");
            st.fail += 1;
            st.record("FAIL", "cargo-test-elymalloc-core");
        }
    }
