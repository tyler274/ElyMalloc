...
pub fn run() -> Result<()> {
    ...
    let unknown = target_dir.join("wasm32-unknown-unknown/release/elymalloc-wasm-smoke.wasm");
    if !unknown.is_file() {
    ...
    let wasi = target_dir.join("wasm32-wasip1/release/elymalloc-wasm-smoke.wasm");
    if !wasi.is_file() {
        bail!("missing {wasi:?}");
    }
    ...
    println!("==> cargo test elymalloc-core --target wasm32-wasip1");
    if std::env::var_os("CARGO_TARGET_WASM32_WASIP1_RUNNER").is_none() {
        std::env::set_var("CARGO_TARGET_WASM32_WASIP1_RUNNER", &wasmtime);
    }
    let st = cargo_in_root()
        .args(["test", "-p", "elymalloc-core", "--target", "wasm32-wasip1"])
        .status()?;
    if !st.success() {
        bail!("elymalloc-core wasm tests failed");
    }
    let st = cargo_in_root()
        .args([
            "test",
        ...
        .status()?;
    if !st.success() {
        bail!("elymalloc-wasm-smoke tests failed");
    }
