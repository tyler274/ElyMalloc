...
fn alloc_stress_bin(dest: &Path) -> PathBuf {
    let p = dest.join("release/elymalloc-alloc-stress");
    if p.is_file() {
        return p;
    }
    dest.join("release/elymalloc-alloc-stress.exe")
}

/// Link `elymalloc-alloc-stress` with each available linker and run it (no preload).
pub fn run_global_alloc_per_linker(cache: &Path) -> Result<()> {
    println!("==> GlobalAlloc binary linked with each linker");
    crate::process::cargo_ok(&["test", "-p", "elymalloc-alloc-stress"])?;
