...
//!
//! Clone upstream, inject `elymalloc_core::ElyMalloc` as `#[global_allocator]` into
...
//! Also builds and runs in-tree `elymalloc-leptos-smoke` (reactive_graph churn).
//!
...
pub fn inject_mimalloc(crate_dir: &Path, elymalloc_core: &Path) -> Result<bool> {
    ...
    let dep = format!(
        "\n[dev-dependencies.elymalloc-core]\npath = \"{}\"\n",
        path_toml(elymalloc_core)
    );
    if !toml.contains("[dev-dependencies.elymalloc-core]") {
        toml.push_str(&dep);
        fs::write(&cargo, toml)?;
    }
    if !rs.ends_with('\n') {
    ...
    rs.push_str(
        "\n#[cfg(test)]\n#[global_allocator]\nstatic __MIMALLOC_REWRITE: elymalloc_core::ElyMalloc = elymalloc_core::ElyMalloc;\n",
    );
    fs::write(&lib, rs)?;
    Ok(true)
}
...
fn run_in_tree_smoke(wasmtime: &Path) -> Result<()> {
    println!("==> elymalloc-leptos-smoke (reactive_graph + ElyMalloc) wasm32-wasip1");
    let st = cargo_in_root()
        .args([
            "build",
            "--release",
            "-p",
            "elymalloc-leptos-smoke",
            "--target",
            "wasm32-wasip1",
        ])
        .status()?;
    if !st.success() {
        bail!("elymalloc-leptos-smoke wasip1 build failed");
    }
    let wasm = rust_root().join("target/wasm32-wasip1/release/elymalloc-leptos-smoke.wasm");
    if !wasm.is_file() {
    ...
    if !st.success() {
        bail!("wasmtime elymalloc-leptos-smoke wasip1 failed");
    }

    println!("==> elymalloc-leptos-smoke wasm32-unknown-unknown");
    let st = cargo_in_root()
        .args([
            "build",
            "--release",
        ...
        .status()?;
    if !st.success() {
        bail!("elymalloc-leptos-smoke unknown-unknown build failed");
    }
    let unknown =
        rust_root().join("target/wasm32-unknown-unknown/release/elymalloc-leptos-smoke.wasm");
    if !unknown.is_file() {
...
pub fn run() -> Result<()> {
    ...
    println!("==> cargo test -p elymalloc-leptos-smoke (host)");
    let st = cargo_in_root()
        .args(["test", "-p", "elymalloc-leptos-smoke"])
        .status()?;
    if !st.success() {
        bail!("elymalloc-leptos-smoke host tests failed");
    }

    run_in_tree_smoke(&wasmtime)?;

    let src = ensure_leptos_src()?;
    let core = rust_root().join("crates/elymalloc-core");
    ...
    println!("==> injected ElyMalloc GlobalAlloc into {injected} leptos crates");
...
mod tests {
    ...
    fn inject_is_idempotent() {
        ...
        assert_eq!(
            toml.matches("[dev-dependencies.elymalloc-core]").count(),
            1,
            "{toml}"
        );
        let _ = fs::remove_dir_all(&dir);
