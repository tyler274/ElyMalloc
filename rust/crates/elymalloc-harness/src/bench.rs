...
pub fn run() -> Result<()> {
    ...
    let ga = rust.join("target/release/elymalloc-bench");
    println!("==> cargo build -p elymalloc-bench");
    crate::process::cargo_ok(&["build", "--release", "-p", "elymalloc-bench"])?;
    if ga.is_file() {
