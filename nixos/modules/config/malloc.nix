# Drop-in replacement for nixpkgs `nixos/modules/config/malloc.nix`.
# Upstream PR delta vs nixos-unstable:
#   1. `elymalloc` provider (`pkgs.elymalloc`) — ElyMalloc, a pure-Rust
#      mimalloc v3 ABI drop-in. Option value is lowercase like `jemalloc`.
#   2. Providers may set `libPaths` (list) instead of `libPath` so more than
#      one SONAME can be written to `ld-nix.so.preload` (ElyMalloc ships both
#      `libmimalloc.so` and `libmimalloc-secure.so.3`; nixpkgs mold DT_NEEDs
#      the latter).
{
  config,
  lib,
...
let
  ...
  providers = {
    ...
    elymalloc = {
      libPaths = [
        "${pkgs.elymalloc}/lib/libmimalloc.so"
        "${pkgs.elymalloc}/lib/libmimalloc-secure.so.3"
      ];
      description = ''
        ElyMalloc, a pure-Rust rewrite of mimalloc with the C mimalloc v3
        ABI (`libmimalloc.so.3`, `mi_*`). Mitigations matching C
        `-DMI_SECURE=FULL` are always enabled. Both SONAMEs are preloaded so
        binaries that `DT_NEEDED` `libmimalloc-secure.so.3` (for example
        nixpkgs mold) bind ElyMalloc instead of C mimalloc.
      '';
    };
  };
  ...
  libPaths = providerConf.libPaths or [ providerConf.libPath ];
  ...
  mallocLib =
    pkgs.runCommand "malloc-provider-${cfg.provider}"
      ...
      ''
        mkdir -p $out/lib
        for p in $origLibPaths; do
          cp -L "$p" "$out/lib/$(basename "$p")"
        done
      '';

  providerPreloadText = lib.concatMapStringsSep "\n" (p: "${mallocLib}/lib/${baseNameOf p}") libPaths;
in
